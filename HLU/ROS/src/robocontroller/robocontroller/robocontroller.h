#ifndef _ROBOCONTROLLER_NODE_H_
#define _ROBOCONTROLLER_NODE_H_
...
public:
    ...
    vector<uint16_t> readMultiReg(uint16_t startAddr, uint16_t nReg ); ///< Called to read registers from RoboController

    // Qvector used instead of QList to provide direct data access using "Data()" function
    bool writeMultiReg( uint16_t startAddr, uint16_t nReg, vector<uint16_t> vals ); ///< Called to write registers to RoboController
